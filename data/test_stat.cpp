...
#define maxRandom random::getRandomInt(0, 60000)
#define randomHP doRandom * random::getRandomFloat4f(10, 50)
#define randomATK doRandom * random::getRandomFloat4f(5, 10)
...
int main() {
    ...
    stat player = stat(floor + randomHP, randomATK);
    player.setHP_percent(maxRandom);
    player.setATK_percent(maxRandom);
    stat enemy = stat(floor + randomHP, randomATK);
    enemy.setHP_percent(maxRandom);
    enemy.setATK_percent(maxRandom);
    ...
    // player.setAP_percent(maxRandom);
    // player.setDEF_percent(maxRandom);
    // player.setACC_percent(maxRandom);
    // player.setEVA_percent(maxRandom);
    // player.setCR_percent(maxRandom);
    // player.setCDMG_percent(maxRandom);
    // player.setCDEF_percent(maxRandom);
    player.toString();
    ...
    // enemy.setAP_percent(maxRandom);
    // enemy.setDEF_percent(maxRandom);
    // enemy.setACC_percent(maxRandom);
    // enemy.setEVA_percent(maxRandom);
    // enemy.setCR_percent(maxRandom);
    // enemy.setCDMG_percent(maxRandom);
    // enemy.setCDEF_percent(maxRandom);
    enemy.toString();
    ...
    while (player.getFinalHP() > 0 && enemy.getFinalHP() > 0) {
        ...
        if (playerTurn) {
            ...
            cout << "player" << " Dealt : " << damage.attack<uint32_t>(player, enemy) << 
            (damage.dodge.isDodge() ? " --dodge" : "        ") << (damage.crit.isCrit() ? " crit!!" : "       ") << "           .. ";
            cout << "enemy" << "  : " << enemy.getFinalHP() << "/" << enemy.getFinalMAX_HP() << " [" 
        ...
        if (enemyTurn) {
            playerTurn = true;
            enemyTurn = false;
            cout << "enemy" << " Dealt  : " << damage.attack<uint32_t>(enemy, player) << 
            (damage.dodge.isDodge() ? " --dodge" : "        ") << (damage.crit.isCrit() ? " crit!!" : "       ") << "           ,, ";
            cout << "player" << " : " << player.getFinalHP() << "/" << player.getFinalMAX_HP() << 
    ...
    cout << "player" << " -- min : " << ((min_pdmg == UINT64_MAX) ? 0 : min_pdmg) << ", max : " << max_pdmg << endl;
    cout << "enemy" << " -- min : " << ((min_edmg == UINT64_MAX) ? 0 : min_edmg) << ", max : " << max_edmg << endl;
